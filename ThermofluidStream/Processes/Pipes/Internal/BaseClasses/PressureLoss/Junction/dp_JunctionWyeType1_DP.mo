...
calculated for case 1 (alpha &lt;= 60&deg;) as: <code>zeta_cb = 1 + w_relbc^2 - 2*(1-*Q_relbc)^2 * k * F_relcb*(Q_relbc)^2</code>
</p>
...
<p>
For case 2 (alpha = 90&deg;): <code>zeta_cb = A * [A + w_relbc^2 - 2*(1 - Q_relbc)^2] = A * zeta'_cb</code>, with coefficient A scaling zeta'_cb with respect to the given area ratio F_relbc. 
...
<p>
For case 3 (60&deg; &lt; alpha &lt;90&deg;): Since there is no aproximation formula nor data available in this case zeta_cb is interpolated linear between the function of case 1 and 2.
The <strong>pressure loss coefficient of the straight channel zeta_cs</strong> is calculated as:
</p>

<ul>
  <li>case 1 (alpha &lt;= 60&deg;): <code>zeta_cs = 1 - (1 - Q_relbc)^2 - k * F_relbc * Q_relbc^2</code>, where k is the same coefficient discribed above. </li>
  <li>case 2 (alpha = 90&deg;): <code>zeta_cs = 1.55 * Q_relbc - Q_relbc^2</code></li>
  <li>case 3 (60&deg; &lt; alpha &lt;90&deg;): As before linear interpolation between the function at alpha &lt;= 60 and alpha = 90 is used to determine the pressure loss in case 3.</li>
</ul>
<p>
The following figures, pressure loss coefficients of the branching channel <strong>Fig.2</strong> and straight channel <strong>Fig.3</strong> at alpha = 30&deg; are shown. (Currently not yet available)
</p>

<p>
The following figures pressure loss coefficients of the branching channel <strong>Fig.4</strong> and straight channel <strong>Fig.5</strong> at alpha = 45&deg; are shown. (Currently not yet available)
</p>

<p>
The following figures pressure loss coefficients of the branching channel <strong>Fig.6</strong> and straight channel <strong>Fig.7</strong> at alpha = 60&deg;are shown. (Currently not yet available) 
</p>

<p>
The following figures pressure loss coefficients of the branching channel <strong>Fig.8</strong> and straight channel <strong>Fig.9</strong> at alpha = 90&deg; are shown. (Currently not yet available)
</p>

<p>
The following figures pressure loss coefficients of the branching channel <strong>Fig.10</strong> and straight channel <strong>Fig.11</strong> at alpha = 75&deg; are shown. (Currently not yet available)
</p>
